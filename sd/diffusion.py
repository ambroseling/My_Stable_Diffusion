...
class UNET_AttentionBlock(nn.Module):
    ...
    def forward(self,x,context):
        ...
        #normalize + FF with GeLU and skip connection
...
class UNET(nn.Module):
    def __init__(self):
        ...
        self.decoders = nn.ModuleList([
            SwitchSequential(UNET_ResidualBlock(2560,1280)),# due to skip connections num of channels is doubled
...
class Diffusion(nn.Module):
    ...
    def forward (self,latent:torch.Tensor,context:torch.Tensor,time:torch.Tensor):
        # latent: (B,4,H/8,W/8)
        # context or prompt: (B,seq_len,dim)
        # time: (1,320)
